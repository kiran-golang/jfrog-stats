...
func GetTLSConfig() (*tls.Config, error) {
	...
	caCert, err := ioutil.ReadFile(GetConfiguration().CertAuthorityFile)
	...
	certPEMBlk, err := readPEMBlock(GetConfiguration().ServerCertificate)
	if err != nil {
		return nil, pkgerrors.Wrap(err, "Read Cert File")
	}

	keyPEMBlk, err := readPEMBlock(GetConfiguration().ServerPrivateKey)
	if err != nil {
